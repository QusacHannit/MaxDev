"""
MaxDev Platform - Flask Backend (отправка писем через Brevo HTTP API)
"""
...
def send_email_via_brevo(to: str, subject: str, html: str) -> bool:
    ...
    payload = {
        'sender': {'name': 'MaxDev', 'email': SENDER_EMAIL},
        'to': [{'email': to}],
        'subject': subject,
        'htmlContent': html,
    }
    try:
        ...
        if response.status_code in (200, 201):
            ...
            print(f'[EMAIL ERROR] Brevo ответил {response.status_code}: {response.text}')
            return False
...
app.config['SQLALCHEMY_DATABASE_URI'] = os.getenv('DATABASE_URL', 'sqlite:///maxdev.db')
...
def send_verification_code():
    ...
    ok = send_email_via_brevo(email, subject, html)

    if not ok:
        return jsonify({'error': 'Не удалось отправить письмо. Проверьте настройки Brevo.'}), 500

    add_log('EMAIL_CODE', None, f'Sent {purpose} code to {email}')
    return jsonify({'message': 'Код отправлен на ваш email', 'dev': not bool(BREVO_API_KEY)})
...
def create_job_payment(job_id):
    ...
    try:
        response = yookassa_request(
            ...
            '/payments',
            payload={
                'amount': {'value': f'{job.budget:.2f}', 'currency': 'RUB'},
                'capture': True,
                'confirmation': {'type': 'redirect', 'return_url': build_payment_return_url(job.id)},
